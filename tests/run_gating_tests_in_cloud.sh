...
cp linchpin/inventories/*.inventory inventory/linchpin.inventory

### Configure remote user for playbooks
# It can be fedora or cloud-user for cloud images
for USER in root fedora cloud-user; do
  ansible-playbook --extra-vars="remote_user=$USER" check_and_set_remote_user.yml
done

### Preparet test runner
ansible-playbook prepare-test-runner.yml
### Run test on test runner (testing system would supply artifacts variable)
